...
class Cpsi_Gateway extends WC_Payment_Gateway {
    ...
    public function handleAdminFormPost() {
        ...
        $oldOptions =get_option(CpsiKeys::OPTIONS_ID);
        if(!is_array($oldOptions)) {$oldOptions = array();};
        
        $options = array_merge($oldOptions, array(
            CpsiKeys::OPTION_CORVUSSTORE_ID => '',
            CpsiKeys::OPTION_CORVUS_KEY => '',
            CpsiKeys::OPTION_DEBUG => '',
            CpsiKeys::OPTION_CHECKOUT_PAGE_ID => '',
            CpsiKeys::OPTION_SUCCESS_PAGE_ID => '',
            CpsiKeys::OPTION_CANCEL_PAGE_ID => '',
            CpsiKeys::OPTION_ENABLE_INSTALLMENTS => "no",
            CpsiKeys::OPTION_REQUIRE_COMPLETE => "yes"
        ));
        
        foreach ($options as $key => $val) {
            switch ($key) {
                case CpsiKeys::OPTION_CORVUSSTORE_ID:
                    $options[CpsiKeys::OPTION_CORVUSSTORE_ID] = $this->get_option($key, $val);
                    break;
                case CpsiKeys::OPTION_CORVUS_KEY:
                    $options[CpsiKeys::OPTION_CORVUS_KEY] = $this->get_option($key, $val);
                    break;
                case CpsiKeys::OPTION_DEBUG:
                    $options[CpsiKeys::OPTION_DEBUG] = $this->get_option($key, $val) == "yes" ? true : false;
                    break;
                case CpsiKeys::OPTION_CHECKOUT_PAGE_ID:
                    $options[CpsiKeys::OPTION_CHECKOUT_PAGE_ID] = $this->get_option($key, $val);
                    break;
                case CpsiKeys::OPTION_SUCCESS_PAGE_ID:
                    $options[CpsiKeys::OPTION_SUCCESS_PAGE_ID] = $this->get_option($key, $val);
                    break;
                case CpsiKeys::OPTION_CANCEL_PAGE_ID:
                    $options[CpsiKeys::OPTION_CANCEL_PAGE_ID] = $this->get_option($key, $val);
                    break;
                case CpsiKeys::OPTION_ENABLE_INSTALLMENTS:
                    $options[CpsiKeys::OPTION_ENABLE_INSTALLMENTS] = $this->get_option($key, $val) == "yes" ? true : false;
                    break;
                case CpsiKeys::OPTION_REQUIRE_COMPLETE:
                    $options[CpsiKeys::OPTION_REQUIRE_COMPLETE] = $this->get_option($key, $val) == "yes" ? true : false;
                    break;
            }
        }
